	...
	, _meshWindow(nullptr)
{
	...
	//m_renderer.getPlugin3D().getMainCamera().position = glm::vec3(m_room.getRoomWidth() - 1.f, m_room.getRoomWidth() - 1.f, m_room.getRoomWidth() - 1.f);
	...
	//m_renderer.getPlugin3D().getMainCamera().rotation = glm::vec3(-M_PI_4, M_PI_4, 0);

	columnHeight = 8;
	selectionAABB = AABB3D();

	//m_triangleMesh = CUSTOM_NEW(InstancedTriangleMesh, m_allocator)(VertexDataType::InstancedTextureMeshVerts, m_renderer, m_allocator);
	//m_meshInstanceID = m_triangleMesh->addInstance();
	//m_triangleMesh = CUSTOM_NEW(TriangleMesh, m_allocator)(VertexDataType::MeshVerts, m_renderer);	
	//_mesh = CUSTOM_NEW(InstancedMesh, m_allocator)(m_renderer, m_allocator);
	//_instanceID = _mesh->addInstance(glm::vec3(0,2,0));
	_voxels = CUSTOM_NEW(VoxelData, m_allocator)(8, 8, 8, m_allocator);
...
void Object3DEditor::Draw()
{
	...
	//};
	//m_renderer.getPlugin3D().bufferCubes(&tableCube, 1);
...
void Object3DEditor::DrawEditCursor()
{
	...
	//m_cursor.posWorld = m_renderer.getPlugin3D().GetCursor3DPos(m_cursor.posScrn);
...
void Object3DEditor::refreshVoxelMesh()
{
	...
	//TexturedPBRVertexData* verts = _voxels->createTriangleMeshReduced(m_renderer, vertCount, VOXEL_RADIUS);
	//m_triangleMesh->buffer(verts, vertCount);
}
