...
void ObjectWindow::setupMaterialOptions()
{
	...
	const TextureID albedoID = m_renderCore.getTextureID(path, false);
...
void ObjectWindow::onMaterialPickerButton(bool state)
{
	if (!m_materialPicker)
	{
		m_materialPicker = m_gui.createCustomNode<MaterialPicker>(m_gui, m_materials, glm::vec2(512, 512));
