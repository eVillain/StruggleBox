...
PhysicsDebug::PhysicsDebug(Renderer3D& renderer3D)
...
void PhysicsDebug::drawLine(const btVector3& from,const btVector3& to,
    ...
    const Color aC = RGBAColor(fromColor.getX(), fromColor.getY(), fromColor.getZ(), 1.0f);
    const Color bC = RGBAColor(toColor.getX(), toColor.getY(), toColor.getZ(), 1.0f);
    const glm::vec3 a = glm::vec3(from.getX(),from.getY(),from.getZ());
    const glm::vec3 b = glm::vec3(to.getX(),to.getY(),to.getZ());
    ColoredVertex3DData* verts = m_renderer.bufferColoredLines(2);
    verts[0] = { a, aC };
    verts[1] = { b, bC };
}
...
void PhysicsDebug::drawContactPoint(const btVector3& pointOnB,const btVector3& normalOnB,
    ...
    ColoredVertex3DData* verts = m_renderer.bufferColoredLines(2);
    verts[0] = { glm::vec3(from.getX(), from.getY(), from.getZ()), col };
    verts[1] = { glm::vec3(to.getX(), to.getY(), to.getZ()), col };
    //m_renderer->getPlugin3D().Buffer3DLine(glm::vec3(from.getX(), from.getY(), from.getZ()),
