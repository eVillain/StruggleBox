...
public:
	...
	MaterialDef& operator[] (uint8_t id) { return _data[id]; }
	const std::string& getName(const uint8_t id) { return _names[id]; };
	void setName(const uint8_t id, std::string& name) { _names[id] = name; };
...
private:
	MaterialDef _data[256];
	std::string _names[256];
};
