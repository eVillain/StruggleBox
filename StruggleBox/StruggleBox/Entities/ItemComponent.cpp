...
_particles(particles)
{
    ...
    if ( !_owner->HasAttribute("ownerID") ) {
        _owner->GetAttributeDataPtr<int>("ownerID") = 0;
    }
                              ...
                              glm::vec3 position)
{
    ...
    if ( ownerIDA == 0 ) {     // Item without owner, if moving fast it's probably thrown
    ...
    if ( aDamage != 0 ) 
    {   // Calculate damage from velocity
        ...
        if ( velDamage > 1.0f ) 
        {
            ...
            if ( bType == ENTITY_ITEM )
            {   // Item collided with another item
                // Item on item collision, throw sparks!
                ParticleSystemID systemID = _particles.create(FileUtil::GetPath().append("Data/Particles/"), "Sparks3D.plist");
                ParticleSystem* pSys = _particles.getSystemByID(systemID);
                pSys->setPosition(position);
            }
            else if ( bType == ENTITY_HUMANOID )
            {
                ParticleSystemID systemID = _particles.create(FileUtil::GetPath().append("Data/Particles/"), "Blood3D.plist");
                ParticleSystem* pSys = _particles.getSystemByID(systemID);
                pSys->setPosition(position);
                pSys->setDuration(velDamage / 10.0f);
            }
            else if ( bType == ENTITY_DECOR )
            {
                //PhysicsComponent* pComp = (PhysicsComponent*)_entityManager.getComponent(ownerIDB, "Physics");
                //if ( pComp )
                //{
