...
Particle3DEditor::Particle3DEditor(
    ...
    , m_particleMenu(nullptr)
    , m_fileSelectMenu(nullptr)
    ...
    , m_particleSys(nullptr)
{
    ...
    if (injector.hasMapping<Renderer2D>())
    {
        ...
        m_renderer2D->getDefaultCamera().setPosition(glm::vec2(resolution.x / 2.f, resolution.y / 2.f));
    }

    m_timeScaler = 1.0f;

    m_renderer.getDefaultCamera().setPosition(glm::vec3(16, 16, 16));
    m_renderer.getDefaultCamera().setTargetPosition(glm::vec3(16, 16, 16));
...
Particle3DEditor::~Particle3DEditor()
{
    m_particles.destroy(m_particleSystemID);
}
...
void Particle3DEditor::LoadSystem(const std::string& fileName)
{
	if (fileName.length() > 0)
	{
		...
		m_particleSystemID = m_particles.create(particlePath, shortFileName);
        m_particleSys = m_particles.getSystemByID(m_particleSystemID);
        m_particleSys->setActive(true);
		RefreshParticleMenu();
	}
}
