...
std::string StringUtil::Format(const char* format, ...)
{
    ...
    std::string ret = StringUtil::FormatVA(format, args);
    va_end(args);
    return ret;
}

std::string StringUtil::FormatVA(const char* format, va_list ap)
{
...
std::string StringUtil::SizeToString(size_t number) {
    std::stringstream ss;//create a stringstream
    ss << number;//add number to the stream
    return ss.str();//return a string with the contents of the stream
}
std::string StringUtil::TimeFormatMinutesSeconds(const float time)
{
    const int minutes = (int)time / 60;
    const int seconds = (int)time - (minutes * 60);
    std::ostringstream buff;
    buff.precision(0);
    buff << minutes;
    buff << ":";
...
std::string StringUtil::TimeFormatMinutesSecondsMS(const float time)
{
    const int minutes = (int)time / 60;
    const int seconds = (int)time - (minutes * 60);
    const float remainder = time - ((minutes * 60) + seconds);
    std::ostringstream buff;
    buff.precision(3);
    buff << minutes;
    buff << ":";
