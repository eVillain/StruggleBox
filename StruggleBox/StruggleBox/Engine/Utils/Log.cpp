...
void Log::Debug(const char * format, ...)
{
    ...
    const std::string debugMessage = StringUtil::FormatVA(format, args);
    va_end (args);
...
void Log::Info(const char * format, ...)
{
    ...
    va_list args;
    va_start (args, format);
    const std::string infoMessage = StringUtil::FormatVA(format, args);
    va_end (args);
    for (auto output : _outputs)
    {
...
void Log::Warn(const char * format, ...)
{
    ...
    va_list args;
    va_start (args, format);
    const std::string warnMessage = StringUtil::FormatVA(format, args);
    va_end (args);
    for (auto output : _outputs)
    {
...
void Log::Error(const char * format, ...)
{
    ...
    va_list args;
    va_start (args, format);
    const std::string errorMessage = StringUtil::FormatVA(format, args);
    va_end (args);
    for (auto output : _outputs)
    {
