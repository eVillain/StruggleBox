...
TextAtlas* TextAtlasLoader::load(const std::string& filename, const uint8_t fontHeight, Allocator& allocator, TextureCache& textureCache)
{
    ...
    //Log::Info("[TextAtlasLoader::load] Loading file: %s", filename.c_str());
    
    FT_Face face = NULL;
    FT_Error err = FT_New_Face(s_freeType, filename.c_str(), 0, &face);
    if(err)
    {
        Log::Error("[TextAtlasLoader::load] Could not open font: %s - error: %i", filename.c_str(), err);
        return NULL;
    }
    ...
    CHECK_GL_ERROR();
    readGlyphs(face, fontHeight, width, height, atlas->m_glyphs);
    CHECK_GL_ERROR();
