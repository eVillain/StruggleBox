...
void SpriteNode::draw(GUI& gui, const glm::vec3& parentPosition, const glm::vec2& parentScale)
{
	...
	if (!m_enable9Slice)
	{
		bufferQuad(bottomLeft, topRight, glm::vec2(m_texRect.x, m_texRect.y), glm::vec2(m_texRect.right(), m_texRect.top()), positionZ, gui);
	}
	else
	{
		...
		bufferQuad(bottomLeft, slice1, glm::vec2(m_texRect.x, m_texRect.y), slice1Tex, positionZ, gui);
		bufferQuad(glm::vec2(slice1.x, bottomLeft.y), glm::vec2(slice2.x, slice1.y), glm::vec2(slice1Tex.x, m_texRect.y), glm::vec2(slice2Tex.x, slice1Tex.y), positionZ, gui);
		bufferQuad(glm::vec2(slice2.x, bottomLeft.y), glm::vec2(topRight.x, slice1.y), glm::vec2(slice2Tex.x, m_texRect.y), glm::vec2(m_texRect.right(), slice1Tex.y), positionZ, gui);

		bufferQuad(glm::vec2(bottomLeft.x, slice1.y), glm::vec2(slice1.x, slice2.y), glm::vec2(m_texRect.x, slice1Tex.y), glm::vec2(slice1Tex.x, slice2Tex.y), positionZ, gui);
		bufferQuad(glm::vec2(slice1.x, slice1.y), glm::vec2(slice2.x, slice2.y), glm::vec2(slice1Tex.x, slice1Tex.y), glm::vec2(slice2Tex.x, slice2Tex.y), positionZ, gui);
		bufferQuad(glm::vec2(slice2.x, slice1.y), glm::vec2(topRight.x, slice2.y), glm::vec2(slice2Tex.x, slice1Tex.y), glm::vec2(m_texRect.right(), slice2Tex.y), positionZ, gui);

		bufferQuad(glm::vec2(bottomLeft.x, slice2.y), glm::vec2(slice1.x, topRight.y), glm::vec2(m_texRect.x, slice2Tex.y), glm::vec2(slice1Tex.x, m_texRect.top()), positionZ, gui);
		bufferQuad(glm::vec2(slice1.x, slice2.y), glm::vec2(slice2.x, topRight.y), glm::vec2(slice1Tex.x, slice2Tex.y), glm::vec2(slice2Tex.x, m_texRect.top()), positionZ, gui);
		bufferQuad(glm::vec2(slice2.x, slice2.y), glm::vec2(topRight.x, topRight.y), glm::vec2(slice2Tex.x, slice2Tex.y), glm::vec2(m_texRect.right(), m_texRect.top()), positionZ, gui);
	}
...
void SpriteNode::bufferQuad(
	...
	GUI& gui) const
{
	TexturedVertex3DData* verts = gui.bufferTexturedTriangles(6, m_textureID);
