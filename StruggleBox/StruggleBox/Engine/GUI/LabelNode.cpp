...
void LabelNode::draw(GUI& gui, const glm::vec3& parentPosition, const glm::vec2& parentScale)
{
    if (m_text.empty())
    {
    ...
    const TextAtlasID atlasID = gui.getRenderCore().getTextAtlasID(m_font, m_fontHeight);
    const TextAtlas* atlas = gui.getRenderCore().getTextAtlasByID(atlasID);
    ...
    TexturedVertex3DData* verts = gui.bufferTextTriangles(visible_glyphs * 6, atlas->getTextureID());
