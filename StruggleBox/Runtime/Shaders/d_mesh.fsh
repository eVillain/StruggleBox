...
#ifdef GRID_LINES
float getGridLine(vec3 normal, vec3 posWS)
{
    ...
    if (abs(normal.x) > 0.9) {
        float gridLineY = mod(posWS.y*2.0,1.0);
        ...
        float gridLineZ = mod(posWS.z*2.0,1.0);
        ...
        float gridLineX = mod(posWS.x*2.0,1.0);
...
void main(void) {
    ...
    albedo *= getGridLine(normal, fragment.fragPos);
#endif
