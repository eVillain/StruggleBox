...
public:
	...
	std::vector<InstanceTransformData3D> getInstanceData(
		const std::string& animation,
		const float time,
...
private:
	...
	InstanceTransformData3D getFrameAtTime(AnimationStream& stream, const float time);
