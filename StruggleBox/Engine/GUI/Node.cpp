...
void Node::draw(GUI& gui, const glm::vec3& parentPosition, const glm::vec2& parentScale)
{
	const Rect2D bb = getBoundingBox();
	const glm::vec3 relativePosition = parentPosition + glm::vec3(bb.x, bb.y, m_position.z + 1.f);
	const glm::vec2 relativeScale = parentScale * m_scale;

	for (Node* child : m_children)
	{
		child->draw(gui, relativePosition, relativeScale);
	}
}
