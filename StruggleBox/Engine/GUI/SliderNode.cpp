...
SliderNode::SliderNode(
	...
	, m_floatValue(50.f)
{
	m_contentSize = glm::vec2(100.f, 20.f);
	m_backgroundSprite = gui.createSpriteNode(backgroundFrame);
	if (m_backgroundSprite)
	{
		m_backgroundSprite->setEnable9Slice(true);
		m_backgroundSprite->setContentSize(glm::vec2(m_contentSize.x, m_backgroundSprite->getContentSize().y));
	...
	if (m_sliderSprite)
	{
		...
		m_sliderSprite->setContentSize(glm::vec2(m_sliderSprite->getContentSize().x, m_contentSize.y));
		m_sliderSprite->setPositionZ(2.f);
...
void SliderNode::onHighlight(bool inside, const glm::vec2& relativeCursorPosition)
{
	...
	const Rect2D boundingBox = m_sliderSprite->getBoundingBox();
...
{
	const float sliderWidth = m_sliderSprite->getContentSize().x;
	const float sliderPosX = m_sliderSprite->getPosition().x - (sliderWidth / 2.f);
	const float maxWidth = m_contentSize.x - sliderWidth;
	const float sliderRatio = MathUtils::Clamp(sliderPosX / maxWidth, 0.f, 1.f);
	...
	{
		m_intValue = m_intMin + int32_t(sliderRatio * (m_intMax - m_intMin));
		if (m_intCallback)
		{
...
void SliderNode::updateSliderPosition()
{
	...
	if (m_type == Type::FLOAT)
	{
		valueRatio = MathUtils::Clamp((m_floatValue - m_floatMin) / (m_floatMax - m_floatMin), 0.f, 1.f);
	}
	else
	{
		valueRatio = MathUtils::Clamp((float)(m_intValue - m_intMin) / float(m_intMax - m_intMin), 0.f, 1.f);
	}
	const float sliderPos = (maxWidth * valueRatio) + (sliderWidth / 2.f);
