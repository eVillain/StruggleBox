...
bool Input::Terminate()
{
    ...
    if (_inputEventListeners.size() > 0)
    {
        Log::Warn("[Input::Terminate] still had registered InputEventListeners (%lu in stack)", _inputEventListeners.size());
    }
    ...
    {
        Log::Warn("[Input::Terminate] still had registered TextInputListener");
    }
    _textInputListener = nullptr;
    return true;
}
...
void PrintModifiers( Uint32 mod )
{
    ...
    /* a clearer way.                              */
    if( mod & SDL_KMOD_NUM ) Log::Info( "NUMLOCK " );
    if( mod & SDL_KMOD_CAPS ) Log::Info( "CAPSLOCK " );
    if( mod & SDL_KMOD_LCTRL ) Log::Info( "LCTRL " );
    if( mod & SDL_KMOD_RCTRL ) Log::Info( "RCTRL " );
    if( mod & SDL_KMOD_RSHIFT ) Log::Info( "RSHIFT " );
    if( mod & SDL_KMOD_LSHIFT ) Log::Info( "LSHIFT " );
    if( mod & SDL_KMOD_RALT ) Log::Info( "RALT " );
    if( mod & SDL_KMOD_LALT ) Log::Info( "LALT " );
    if( mod & SDL_KMOD_CTRL ) Log::Info( "CTRL " );
    if( mod & SDL_KMOD_SHIFT ) Log::Info( "SHIFT " );
    if( mod & SDL_KMOD_ALT ) Log::Info( "ALT " );
    if( mod & SDL_KMOD_LGUI ) Log::Info( "LGUI " );
    if( mod & SDL_KMOD_RGUI ) Log::Info( "RGUI " );
    if( mod & SDL_KMOD_MODE ) Log::Info( "MODE " );
    //if( mod & SDL_KMOD_RESERVED ) Log::Info( "RESERVED " );
}
...
void PrintKeyInfo(SDL_KeyboardEvent *key)
{
    ...
    Log::Info("Scancode: 0x%02X", key->scancode);
    /* Print the name of the key */
    Log::Info(", Name: %s", SDL_GetKeyName(key->key));
    /* Print modifier info */
...
bool Input::ProcessInput(const SDL_Event& event)
{
	...
	if (event.type == SDL_EVENT_KEY_DOWN)
	{
		...
		{
			amount = 1.0f;
			input = SDL_GetKeyName(event.key.key);
#if 0
...
bool Input::ProcessTextInput(const SDL_Event &event)
{
    ...
    {
        //Handle backspace
        if(event.key.key == SDLK_BACKSPACE && _inputText.length() > 0)
        {
            //lop off character
            _inputText.pop_back();
            return true;
...
void Input::SetDefaultBindings()
{
    ...
    Bind(KEY_MINUS, InputEvent::Stats);
