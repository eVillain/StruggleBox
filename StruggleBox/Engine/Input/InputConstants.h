#ifndef INPUT_CONSTANTS_H
#define INPUT_CONSTANTS_H

#include <string>

const std::string KEY_ESCAPE = "Escape";
const std::string KEY_RETURN = "Return";
const std::string KEY_DELETE = "Delete";
const std::string KEY_BACKSPACE = "BackSpace";
const std::string KEY_UP = "Up";
const std::string KEY_DOWN = "Down";
const std::string KEY_LEFT = "Left";
const std::string KEY_RIGHT = "Right";
const std::string KEY_SPACE = "Space";
const std::string KEY_W = "W";
const std::string KEY_A = "A";
const std::string KEY_S = "S";
const std::string KEY_D = "D";
const std::string KEY_P = "P";
const std::string KEY_C = "C";
const std::string KEY_E = "E";
const std::string KEY_M = "M";
const std::string KEY_R = "R";
const std::string KEY_I = "I";
const std::string KEY_G = "G";
const std::string KEY_1 = "1";
const std::string KEY_2 = "2";
const std::string KEY_3 = "3";
const std::string KEY_4 = "4";
const std::string KEY_5 = "5";
const std::string KEY_6 = "6";
const std::string KEY_7 = "7";
const std::string KEY_8 = "8";
const std::string KEY_9 = "9";
const std::string KEY_0 = "0";
const std::string KEY_TAB = "Tab";
const std::string KEY_LEFT_SHIFT = "Left Shift";
const std::string KEY_RIGHT_SHIFT = "Left Option";
const std::string MOUSE_BUTTON_1 = "MouseButton1";
const std::string MOUSE_BUTTON_2 = "MouseButton2";
const std::string MOUSE_BUTTON_3 = "MouseButton3";
const std::string MOUSE_WHEEL_Y = "MouseWheelY";



const std::string INPUT_START = "start";
const std::string INPUT_BACK = "back";
const std::string INPUT_PAUSE = "pause";
const std::string INPUT_CONSOLE = "console";
const std::string INPUT_ERASE_RIGHT = "eraseright";
const std::string INPUT_ERASE_LEFT = "eraseleft";
const std::string INPUT_LOOK_UP = "lookup";
const std::string INPUT_LOOK_DOWN = "lookdown";
const std::string INPUT_LOOK_LEFT = "lookleft";
const std::string INPUT_LOOK_RIGHT = "lookright";
const std::string INPUT_MOVE_FORWARD = "moveforward";
const std::string INPUT_MOVE_BACK ="moveback";
const std::string INPUT_MOVE_LEFT = "moveleft";
const std::string INPUT_MOVE_RIGHT = "moveright";
const std::string INPUT_JUMP = "jump";
const std::string INPUT_RUN = "run";
const std::string INPUT_SNEAK = "sneak";
const std::string INPUT_GRAB = "grab";
const std::string INPUT_INVENTORY = "inventory";
const std::string INPUT_SHOOT = "shoot";
const std::string INPUT_SHOOT2 = "shoot2";
const std::string INPUT_SCROLL_Y = "scrollY";
const std::string INPUT_BLOCKS_REPLACE = "replace";
const std::string INPUT_EDIT_BLOCKS = "editblocks";
const std::string INPUT_EDIT_OBJECT = "editobject";
const std::string INPUT_GRAB_CURSOR = "grabcursor";

#endif /* INPUT_CONSTANTS_H */
