...
class VertexDataBufferMap
{
public:
    VertexDataBufferMap(RenderCore& renderCore) : m_renderCore(renderCore) {}
	...
	DataType* buffer(const size_t count, const KeyType key)
	{
        ...
        {
            const size_t nextBufferSize = MathUtils::round_up_to_power_of_2(oldBuffer.count + count);
