...
void UIScrollerListBase::Draw( Renderer *renderer ) {
    ...
    // Inner gradient fill
    renderer->DrawGradientY(Rect2D((float)x, (float)yc+1, (float)w-1, (float)h-1), COLOR_UI_GRADIENT_TOP, COLOR_UI_GRADIENT_BOTTOM);
    // Inside border
    glEnable(GL_BLEND);
    renderer->Draw2DRect(Rect2D(x+0.5f,yc+1.5f,w-2,h-2), COLOR_UI_BORDER_INNER, COLOR_NONE);
    renderer->DrawGradientY(Rect2D((float)x, (float)y+1, (float)w-1, (float)contentHeight-1), COLOR_UI_GRADIENT_TOP, COLOR_UI_GRADIENT_BOTTOM);
