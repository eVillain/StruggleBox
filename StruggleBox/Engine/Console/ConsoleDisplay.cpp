...
void ConsoleDisplay::Show()
{
	...
	Console::setUpdateCallback(std::bind(&ConsoleDisplay::Refresh, this));
...
void ConsoleDisplay::Refresh()
{
	...
	{
		LabelNode* label;
		const std::string& text = Console::_textLines[msgCount - (i + 1)].text;
		if (m_textLabels.size() <= i)
		{
			label = m_gui.createLabelNode(text, GUI::FONT_DEFAULT, 24);
